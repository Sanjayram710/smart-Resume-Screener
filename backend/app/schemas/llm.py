...
class EducationItem(BaseModel):
    degree: str = Field(..., description="Degree or qualification name (e.g. B.S. in Computer Science)")
...
class ExperienceItem(BaseModel):
    ...
    technologies: List[str] = Field(default_factory=list, description="Technologies or tools used in this role")


class ProjectItem(BaseModel):
    name: str = Field(..., description="Project name")
    description: Optional[str] = Field(None, description="Brief summary of the project")
    technologies: List[str] = Field(default_factory=list, description="Technologies utilized in this project")
...
class ResumeExtractionOutput(BaseModel):
    ...
    soft_skills: List[str] = Field(default_factory=list, description="Soft skills / communication / leadership")
    
    education: List[EducationItem] = Field(default_factory=list, description="Educational background")
    work_experience: List[ExperienceItem] = Field(default_factory=list, description="Work experience items")
    ...
    years_of_experience: float = Field(0.0, description="Total verified years of professional experience")
    explicit_skills: List[str] = Field(default_factory=list, description="Skills explicitly listed in the resume")
    inferred_skills: List[str] = Field(default_factory=list, description="Skills inferred from project/work context")
    extraction_warnings: List[str] = Field(default_factory=list, description="Noteworthy ambiguities or missing sections")


class JobExtractionOutput(BaseModel):
    job_title: str = Field(..., description="Target job title")
    company: str = Field(..., description="Hiring company name")
    required_skills: List[str] = Field(default_factory=list, description="Must-have technical and professional skills")
    ...
    education_requirements: List[str] = Field(default_factory=list, description="Required education level (e.g. Bachelor's)")
    certifications: List[str] = Field(default_factory=list, description="Desired or required certifications")
    keywords: List[str] = Field(default_factory=list, description="Key domain concepts and buzzwords")
    important_requirements: List[str] = Field(default_factory=list, description="Critical operational requirements")
    nice_to_have_requirements: List[str] = Field(default_factory=list, description="Optional bonus points")
    extraction_warnings: List[str] = Field(default_factory=list, description="Any ambiguities in the job description")


class CandidateEvaluationOutput(BaseModel):
    candidate_name: str = Field(..., description="Name of candidate evaluated")
    overall_assessment: str = Field(..., description="High-level qualitative summary of candidate fit")
    strengths: List[str] = Field(default_factory=list, description="Top positive qualifications and matches")
    missing_skills: List[str] = Field(default_factory=list, description="Required skills not found in candidate profile")
    partial_matches: List[str] = Field(default_factory=list, description="Skills where candidate has adjacent or partial background")
    experience_assessment: str = Field(..., description="Qualitative critique of career progression and seniority")
